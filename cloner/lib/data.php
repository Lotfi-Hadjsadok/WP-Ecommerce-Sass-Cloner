    ...
    class MUCD_Data
    {
        ...
        public static function db_copy_tables($from_site_id, $to_site_id)
        {
            ...
            if ($from_site_id == MUCD_PRIMARY_SITE_ID) {
                // BUG fix when cloning primary website not cloning all tables.
                // Select all tables except wp_[with_number]_ tables.
                // Exclude all network specific tables.
                $sql_query = $wpdb->prepare(
                    ...
                    '^' . $dbprefix . '[0-9]+',
                    $dbprefix . 'blogs',
                    $dbprefix . 'blog_versions',
                    $dbprefix . 'registration_log',
                    $dbprefix . 'signups',
                    $dbprefix . 'site',
                    $dbprefix . 'sitemeta',
                    $dbprefix . 'usermeta',
        ...
        public static function update($table, $fields, $from_string, $to_string)
        {
            if (is_array($fields) || !empty($fields)) {
                ...
                foreach ($fields as $field) {
                    ...
                    // Check the column type to determine the handling of the query
                    ...
                    // Determine what value to pass based on the field type.
                    // If value is not of string related type pass.
                    if (!in_array($field_type, array(
                        ...
                        $value = '%' . $from_string_like . '%';
                    }

                    $sql_query = $wpdb->prepare('SELECT ' . $field . ' FROM ' . $table . ' WHERE ' . $field . ' LIKE %s', $value);
